...
//                     grid_map_.grid[neighbor.x][neighbor.y] == 1 || closed_list[neighbor.x][neighbor.y]) {
//                     continue;
//                 }
//                 double g_cost = current->g_cost + distance(*current, neighbor);
//                 open_list.push(std::make_shared<Node>(Node(neighbor.x, neighbor.y, g_cost, heuristic({neighbor.x, neighbor.y}, gridGoal), current)));
//             }
//         }
//         // finish
...
//                     Node neighbor(neighbor_x, neighbor_y, 0, 0);
...
public:
    ...
    // // 添加路径验证方法
    // bool validatePath(const std::vector<Eigen::Vector2d>& path) {
    //     if (path.size() < 2) return true;
        
    //     for (size_t i = 0; i < path.size() - 1; ++i) {
    //         if (checkLineCollision(path[i], path[i + 1])) {
    //             return false;
    //         }
    //     }
    //     return true;
    ...
    std::vector<Eigen::Vector2d> findPath(Eigen::Vector2d start, Eigen::Vector2d goal) {
        ...
        //     ROS_WARN("Start or goal position is in collision!");
        //     return {};
        // }
...
private:
    // // 添加碰撞检查方法
    // bool checkCollision(const Eigen::Vector2d& point) {
    //     auto grid_point = worldToGrid(point);
    //     // 检查点是否在地图范围内
    //     if (grid_point.first < 0 || grid_point.first >= width_ ||
    //         grid_point.second < 0 || grid_point.second >= height_) {
    //         return true;  // 超出地图范围视为碰撞
    //     }
    //     return grid_map_.grid[grid_point.first][grid_point.second] == 1;
    // }
    ...
    //     // 沿线段采样检查点
    //     double step = grid_resolution_ * 0.5;  // 采样间隔设为栅格大小的一半
    //     for (double t = 0; t <= length; t += step) {
    //         Eigen::Vector2d point = start + direction * t;
    ...
    //     Eigen::Vector2d current_world = gridToWorld(current.x, current.y);
    ...
    //         // 基本边界检查
    //         if (neighbor_x >= 0 && neighbor_x < width_ && 
    //             neighbor_y >= 0 && neighbor_y < height_) {
                
    //             // 对角线移动时的特殊检查
    //             if (std::abs(dir.first) == 1 && std::abs(dir.second) == 1) {
    //                 // 检查相邻的两个格子
    //                 if (grid_map_.grid[current.x][neighbor_y] == 1 || 
    //                     grid_map_.grid[neighbor_x][current.y] == 1) {
    //                     continue;
    //                 }
    //             }
    ...
    //             if (grid_map_.grid[neighbor_x][neighbor_y] == 0) {
    //                 // 转换为世界坐标进行线段碰撞检查
    //                 Eigen::Vector2d neighbor_world = gridToWorld(neighbor_x, neighbor_y);
    //                 if (!checkLineCollision(current_world, neighbor_world)) {
    //                     Node neighbor(neighbor_x, neighbor_y, 0, 0);
    //                     neighbors.push_back(neighbor);
    //                 }
    //             }
